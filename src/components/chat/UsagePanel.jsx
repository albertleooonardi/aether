  ...
  return (
    <div className="py-1.5">
      <div className="flex items-center gap-2">
        ...
        <span className="flex-1 truncate text-ink/85">
          {p.label} <span className="text-ink/35">· {p.model}</span>
        </span>
        <span className="shrink-0 rounded-full px-1.5 py-0.5 text-[10px]" style={{ background: `${s.color}22`, color: s.color }}>
          {s.label}
        </span>
      </div>
      <div className="mt-0.5 flex items-center justify-between pl-4 text-[11px] text-ink/50">
        <span>{fmt(p.tokens)} tokens · {p.calls} calls</span>
        {p.budget ? <span>{fmt(p.budget)} budget</span> : null}
      </div>
      {p.budget ? (
        <div className="mt-1 ml-4 h-1 rounded-full bg-ink/10">
...
const UsagePanel = ({ usage, active }) => {
  if (!usage) {
    return (
      <div className="border-b border-ink/10 bg-ink/5 px-4 py-3 text-xs text-ink/50">
        Loading usage…
      </div>
    );
  }
  ...
  return (
    <div className="border-b border-ink/10 bg-ink/5 px-4 py-3">
      <div className="mb-1 flex items-center justify-between text-[11px] font-semibold uppercase tracking-wide text-ink/60">
        <span>AI usage · today</span>
        <span className="text-ink/45">{fmt(usage.totalTokens)} tokens</span>
      </div>
